...
#define CONST_FP(num) ConstantFP::get((num), (module.get()))  // 得到常数值的表示,方便后面多次用到
// copy paste from compiler principle course code, 2021 fall
int main() {
    auto module = std::make_unique<Module>("Cminus code");  // module name是什么无关紧要
    auto builder = std::make_unique<IRBuilder>(nullptr, module.get());
    ...
    auto x = GlobalVariable::create("x", module.get(), arrayType, false, initializer);
    // 是否是常量定义（cminus中没有常量概念，应全都是false），初始化常量(ConstantZero类)
    auto y = GlobalVariable::create("y", module.get(), arrayType, false, initializer);
    ...
    // 由函数类型得到函数
    auto gcdFun = Function::create(gcdFunTy, "gcd", module.get());

    // BB.get()的名字在生成中无所谓,但是可以方便阅读
    auto bb = BasicBlock::create(module.get(), "entry", gcdFun.get());

    builder->set_insert_point(bb);  // 一个BB.get()的开始,将当前插入指令点的位置设在bb
    ...
    std::vector<std::shared_ptr<Value>> args;  // 获取gcd函数的形参,通过Function中的iterator
    ...
    auto trueBB = BasicBlock::create(module.get(), "trueBB", gcdFun.get());    // true分支
    auto falseBB = BasicBlock::create(module.get(), "falseBB", gcdFun.get());  // false分支
    auto retBB = BasicBlock::create(module.get(), "", gcdFun.get());  // return分支,提前create,以便true分支可以br
    ...
    builder->create_br(retBB);  // br retBB.get()
    ...
    auto funArrayFun = Function::create(funArrayFunType, "funArray", module.get());
    bb = BasicBlock::create(module.get(), "entry", funArrayFun.get());
    ...
    std::vector<std::shared_ptr<Value>> args1;  //获取funArrayFun函数的形参,通过Function中的iterator
    ...
    trueBB = BasicBlock::create(module.get(), "trueBB", funArrayFun.get());
    falseBB = BasicBlock::create(module.get(), "falseBB", funArrayFun.get());
    ...
    builder->create_br(falseBB);  // 注意在下一个BB.get()之前要Br一下
    ...
    bb = BasicBlock::create(module.get(), "entry", mainFun.get());
    ...
    std::ofstream file("gcd.ll", std::ios::out);
    file << module->print();
    return 0;
}
