...
class Type {
  public:
    ...
    static Type *get_ptrtoint_type(Module *m);
    ...
    static IntegerType *get_int64_type(Module *m);
...
class StructType : public Type {
  public:
    StructType(std::vector<Type *> contained, Module *m) : Type(Type::StructTyID, m), contained_(contained) {}
    static StructType *get(std::vector<Type *> &contained, Module *m);
    Type *get_contained_type(int i) { return contained_[i]; }
    std::vector<Type *> &get_contained_types() { return contained_; }
    unsigned get_num_of_elements() { return contained_.size(); }

  private:
    std::vector<Type *> contained_;
};
