...
bool promote(IRBuilder &builder, std::shared_ptr<Value> l_val, std::shared_ptr<Value> r_val) {
    bool is_int;
...
void CminusfBuilder::visit(ASTFunDeclaration &node) {
    ...
    cur_fun = fun.get();
    auto funBB = BasicBlock::create(module.get(), "entry", cur_fun);
...
void CminusfBuilder::visit(ASTVar &node) {
    ...
    if (node.expression == nullptr) {
        ...
        builder->create_call((std::dynamic_pointer_cast<Function> (neg_idx_except_fun)), {});
        if (cur_fun->get_return_type()->is_void_type())
            builder->create_void_ret();
        else if (cur_fun->get_return_type()->is_float_type())
            builder->create_ret(CONST_FP(0.));
...
void CminusfBuilder::visit(ASTCall &node) {
     auto func = std::dynamic_pointer_cast<Function>(scope.find(node.id));
