...
IntegerType *Type::get_int64_type(Module *m) { return m->get_int64_type(); }

Type *Type::get_ptrtoint_type(Module *m) { return m->get_ptrtoint_type(); }
...
std::string Type::print() {
    ...
    switch (this->get_type_id()) {
        ...
        case StructTyID: {
            ...
            type_ir += "<{ ";
            for (int i = 0; i < struct_type->get_num_of_elements(); i++) {
                type_ir += struct_type->get_contained_type(i)->print();
                if (i < struct_type->get_num_of_elements() - 1)
                    type_ir += ", ";
            }
            type_ir += " }>";
        }
...
IntegerType *IntegerType::get(unsigned num_bits, Module *m) {
    if (num_bits == 1) {
        ...
        if (!("IntegerType::get has error num_bits"))
            exit(231);
...
FunctionType::FunctionType(Type *result, std::vector<Type *> params) : Type(Type::FunctionTyID, nullptr) {
    if (!(is_valid_return_type(result) && "Invalid return type for function!"))
        exit(232);
    result_ = result;

    for (auto &p : params) {
        if (!(is_valid_argument_type(p) && "Not a valid type for function argument!"))
            exit(233);
        args_.push_back(p);
    }
}
...
ArrayType::ArrayType(Type *contained, unsigned num_elements)
    ...
    if (!(is_valid_element_type(contained) && "Not a valid type for array element!"))
        exit(234);
...
std::vector<unsigned int> ArrayType::get_shape() {
    std::vector<unsigned int> ret{};
    Type *current = this;
    while (current->is_array_type()) {
        ret.push_back(static_cast<ArrayType *>(current)->get_num_of_elements());
        current = static_cast<ArrayType *>(current)->get_array_element_type();
    }
    return ret;
}
...
StructType *StructType::get(std::vector<Type *> &contained, Module *m) { return m->get_struct_type(contained); }